requirejs.config({
    ...
    paths: {
        ...
        nextstop: "js/models/nextstop"
    }
});

requirejs(["jquery", "underscore", "backbone", "nextstop", "jqUnit"], function($, _, Backbone, NextStop, jqUnit) {
    var nextstopTests = new jqUnit.TestCase("nextstop module tests"),
        ...
        testModuleFunction = function (messageFunc, tests, moduleFunc) {
            var nextstop = new NextStop();
            expect(tests.length);
            $.each(tests, function(index, test){
                makeEqualTest(messageFunc, nextstop[moduleFunc], test.args, test.expectedValue);
            });
            nextstop.destroy();
            start();
        },
        nextstopPropertyCheck = function (defaultOptions, propertiesToCheck, newTime, newPropertiesToCheck) {
            var nextstop = new NextStop(defaultOptions),
                expectNum,
                propertiesToCheckFunction = function (properties) {
                    $.each(properties, function(name, expectedValue) {
                        jqUnit.assertEquals("".concat(name, " should be equal to ", expectedValue), expectedValue, nextstop.get(name));
                    });
                };
            ...
            if (newTime) {
                nextstop.set({currentTime: newTime});
        ...
        testScenarios = {
            ...
            "nextstop module empty testing": function() {
                nextstopPropertyCheck(null, {
                    currentTime: "0:00",
                    nextTime: undefined,
                    timeLeft: undefined
                });
            },
            "nextstop module non-empty testing": function() {
                nextstopPropertyCheck({
                    currentTime: "1:10",
            ...
            "nextstop module non-empty testing with a time shift": function() {
                nextstopPropertyCheck({
                    currentTime: "1:10",
                    stopTimes: ["1:08", "1:17", "2:25"]
                },
    ...
    $.each(testScenarios, function(message, func) {
        nextstopTests.asyncTest(message, func);
    });
});
